...
IceAdapterOptions IceAdapterOptions::init(int argc, char *argv[])
{
  ...
  try
  {
      ...
      desc.add_options()
        ...
        ("stun-ip",      po::value<std::string>(&result.stunIp)->default_value("37.58.123.3"),         "set the STUN IP address")
        ("turn-ip",      po::value<std::string>(&result.turnIp)->default_value("37.58.123.3"),         "set the TURN IP address")
