...
program
  .version('1.0.0')
  .option('--id', 'set the ID of the local player')
  .option('--login', 'set the login of the local player, e.g. "Rhiza"')
  .option('--rpc-port', 'set the port of internal JSON-RPC server')
  .option('--ice-port-min', 'start of port range to use for ICE local host candidates')
  .parse(process.argv);
