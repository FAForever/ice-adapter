...
static constexpr std::size_t numPeers = 4;
  ...
  for (PeerId localId = 0; localId < numPeers; ++localId)
  {
    for (PeerId remoteId = 0; remoteId < numPeers; ++remoteId)
    {
      if (localId != remoteId)
      {
        ...
        //std::cout << "datachannel " << pc->localId << " -> " << pc->remoteId << " opened" << std::endl;
      }
    }
  }
...
int main(int argc, char *argv[])
{
  ...
  pcfactory = webrtc::CreateModularPeerConnectionFactory(rtc::Thread::Current(),
                                                         rtc::Thread::Current(),
                                                         rtc::Thread::Current(),
                                                         nullptr,
                                                         nullptr,
